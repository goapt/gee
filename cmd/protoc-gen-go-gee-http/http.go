...
const (
	...
	transportHTTPPackage = protogen.GoImportPath("github.com/goapt/gee")
	errorsPackage        = protogen.GoImportPath("github.com/goapt/gee/errors")
)

var methodSets = make(map[string]int)

// generateFile generates a _http.pb.go file containing gee http definitions.
func generateFile(gen *protogen.Plugin, file *protogen.File, omitempty bool) *protogen.GeneratedFile {
	...
	g.P("// Code generated by protoc-gen-go-gee-http. DO NOT EDIT.")
	g.P("// versions:")
	g.P(fmt.Sprintf("// protoc-gen-go-gee-http %s", release))
	g.P()
...
func generateFileContent(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, omitempty bool) {
	...
	g.P("// is compatible with the gee package it is being compiled against.")
	g.P("// ", contextPackage.Ident(""), transportHTTPPackage.Ident(""), httpPackage.Ident(""), errorsPackage.Ident(""))
	g.P()
...
func genService(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, service *protogen.Service, omitempty bool) {
	...
	for _, method := range service.Methods {
		...
		if rule != nil && ok {
			...
			sd.Methods = append(sd.Methods, buildMethodDesc(g, service, method, "POST", path))
		}
	}
	if len(sd.Methods) != 0 {
		g.P(sd.execute())
...
func buildHTTPRule(g *protogen.GeneratedFile, service *protogen.Service, m *protogen.Method, rule *annotations.HttpRule) *methodDesc {
	...
	md := buildMethodDesc(g, service, m, method, path)
...
func buildMethodDesc(g *protogen.GeneratedFile, service *protogen.Service, m *protogen.Method, method, path string) *methodDesc {
	defer func() { methodSets[service.GoName+m.GoName]++ }()
	...
	return &methodDesc{
		Name:    m.GoName,
		Num:     methodSets[service.GoName+m.GoName],
